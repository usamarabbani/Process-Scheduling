...
public class HPF_Pre
{
    ...
    private static void procSwitch(PriorityQueues readyQueues, float currentTime)
    {
        ...
        else if(!readyQueues.isEmpty() && readyQueues.getCurrentProc().getAllotedTime() <= 0)
        {
            readyQueues.updateCurrentProc();
            readyQueues.getCurrentProc().setAllotedTime(MAX_ALLOTED_TIME);
        }
    }
}
